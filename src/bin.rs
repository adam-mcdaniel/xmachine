...
fn add(xasm: &mut Machine) {
    ...
    if let (Some(m), Some(n)) = (first, second) {
        ...
        xasm.push(Value::number(m_f + n_f));
    }
}
...
fn main() {
    ...
    xasm.copy();
    xasm.push(Value::string("Point"));
    xasm.store();
    xasm.push(Value::number(3));
    xasm.copy();
    xasm.push(Value::number(2));
    xasm.copy();
    xasm.push(Value::string("Point"));
    xasm.load();
    xasm.copy();
